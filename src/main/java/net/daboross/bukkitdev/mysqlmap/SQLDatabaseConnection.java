        ...
        private void create() {
            sql.run("create StringTable " + name, new SQLRunnable() {
                ...
                public void run(Connection connection) throws SQLException {
                    String query = String.format("CREATE TABLE IF NOT EXISTS `%s` (`stringKey` VARCHAR, `stringValue` TEXT, PRIMARY KEY (`stringKey`));", name);
    ...
    private class IntTable implements MapTable<String, Integer> {
        ...
        private void create() {
            sql.run("create IntTable " + name, new SQLRunnable() {
                @Override
                public void run(Connection connection) throws SQLException {
                    String query = String.format("CREATE TABLE IF NOT EXISTS `%s` (`stringKey` VARCHAR, `intValue` INT, PRIMARY KEY (`stringKey`))", name);
                    PreparedStatement statement = connection.prepareStatement(query);
                    try {
                        statement.execute();
                    } finally {
